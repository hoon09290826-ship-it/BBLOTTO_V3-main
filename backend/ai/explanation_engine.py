...
# Final grounded formatter: describe only values that can be recalculated from
# the returned combinations.  This prevents a fluent template from claiming a
# HOT/COLD rank, superiority, or balance that the visible numbers do not prove.
def build_round_analysis(round_no: int, stats: Mapping[str, Any] | None, mode: str, fixed: Any, excluded: Any, details: Sequence[Mapping[str, Any]]) -> str:
    valid = _valid_details(details)
    if not valid:
        return "추천 조합을 확인할 수 없어 분석 문구를 만들지 못했습니다."

    combos = [item["numbers"] for item in valid]
    usage = Counter(n for combo in combos for n in combo)
    repeated = sorted(usage.items(), key=lambda item: (-item[1], item[0]))
    repeated = [(number, count) for number, count in repeated if count > 1][:6]
    sums = [sum(combo) for combo in combos]
    odd = sum(n % 2 for combo in combos for n in combo)
    total = len(combos) * 6
    zones = [
        sum(n <= 15 for combo in combos for n in combo),
        sum(16 <= n <= 30 for combo in combos for n in combo),
        sum(n >= 31 for combo in combos for n in combo),
    ]
    overlaps = [
    ...
    line1 = f"{round_no}회차 추천번호 {len(combos)}개 조합을 생성했습니다."
    if repeated:
        repeat_text = ", ".join(f"{number}번 {count}회" for number, count in repeated)
        line2 = f"여러 조합에 포함된 번호는 {repeat_text}입니다."
    else:
        line2 = f"전체 조합에서 사용한 {len(usage)}개 번호는 각각 1회씩 포함됐습니다."
    line3 = (
        f"전체 번호 기준 홀짝은 {odd}:{total - odd}, 구간 분포는 "
        f"{zones[0]}:{zones[1]}:{zones[2]}, 조합 합계는 {min(sums)}~{max(sums)}입니다."
    )
    line4 = (
        f"조합끼리 겹치는 번호는 최대 {max(overlaps, default=0)}개이며, "
        f"연속번호 쌍은 전체 {consecutive}개입니다."
    )
    return "\n".join([line1, line2, line3, line4])
